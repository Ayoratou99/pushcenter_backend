...
class BusinessController extends BaseController
{
    ...
    public function store(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'webhook_events.*' => 'string|in:message.sent,message.failed',
        ]);
        ...
        // Only the hash of the secret is stored, so this response is the one and
        // only place the clear value ever appears.
        ...
        ], 'Application created. Copy the app secret now: it cannot be shown again.');
    }
    ...
    public function regenerateCredentials($id): JsonResponse
    {
        ...
        // Rotating the secret invalidates every token the application holds.
        $business->app_id = \App\Models\Business::generateAppId();
        $appSecret = $business->issueAppSecret();
        $business->save();

        return $this->successResponse([
            'app_id' => $business->app_id,
            'app_secret' => $appSecret,
        ], 'App credentials regenerated. Save the app secret now: it cannot be shown again.');
    }
}
